        ...
        <form method="GET" class="card shadow-sm mb-4">
            <div class="card-body row g-3 align-items-end">
                ...
                <div class="col-md-2">
                    ...
                    <a href="{{ request()->url() }}" class="btn btn-secondary w-100 mt-2">Reset</a>
                </div>
                            ...
                            @forelse($data as $i => $row)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <span
                                            class="badge bg-{{ $row->status == 'approved' ? 'success' : ($row->status == 'pending' ? 'warning text-dark' : 'danger') }}">
                                            {{ ucfirst($row->status) }}
                                        </span>
                                    </td>
                                    <td class="text-center">
                                        @if ($row->file)
                                            @php
                                                $cleanPath = ltrim($row->file, '/');
                                                $fullUrl = asset('storage/' . $cleanPath);
                                                $isImage = in_array(
                                                    strtolower(pathinfo($cleanPath, PATHINFO_EXTENSION)),
                                            ...
                                            @else
                                                <a href="{{ $fullUrl }}" class="btn btn-sm btn-outline-primary"
